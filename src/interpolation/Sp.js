...
import { Table, Button, Calendar, Input, Select, Layout, Card } from 'antd';
import axios from "axios";
const { Header, Content, Footer, Sider } = Layout;
const math = require('mathjs');
const Spline = require('cubic-spline');
let A = [], B = [], C = [], matrixA = [], matrixB = [],dimention=0,number;
function Sp() {
    ...
    for (let i = 0; i < dimention; i++) {
      ...
      A.push(
        <Input
          ...
          id={"a" + p} key={"a" + p} value = {matrixA[i]} />
      )
      C.push(<br />)
      B.push(
        <Input
        style={{
          ...
          id={"b" + p} key={"b" + p} value ={matrixB[i]} />
      )
      A.push(<br />)
      B.push(<br />)
    }
  ...
  function init() {
    ...
    for (var i = 0; i < dimention; i++) {
      matrixA.push(parseFloat(document.getElementById("a" + (i + 1)).value))
    ...
    <Layout style={{background:"white",padding:"0%"}}>
      {sinput && <div>
        <Input
          ...
          name="dimention" onChange={(e) => dimention = e.target.value} />
      ...
      {smatrix && <Layout style={{marginLeft:"10%"}}>
        <Layout>
        ...
        <Layout >
          <Sider style={{ background: "white" }}>
            <div >
              <p style={{ marginLeft: "20%" }}>Value of X</p><br />{A}
            </div>
          </Sider>
          ...
          <Sider style={{ background: "white" }}>
            <div>
              <p style={{ marginLeft: "30%" }}>F(x)</p><br />{B}
            </div>
          </Sider>
        </Layout>
